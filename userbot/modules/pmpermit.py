...
import ..database.pmpermitdb as noobdb
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "off":
        await noobdb.set_pm(False)
        await message.edit("**PM Guard Deactivated**")
    if arg == "on":
        await noobdb.set_pm(True)
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    await noobdb.set_limit(int(arg))
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await noobdb.set_permit_message(noobdb.PMPERMIT_MESSAGE)
        await message.edit("**Anti_PM message set to default**.")
        return
    await noobdb.set_permit_message(f"`{arg}`")
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await noobdb.set_block_message(noobdb.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await noobdb.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await noobdb.get_pm_settings()
    await noobdb.allow_user(chat_id)
...
async def deny(client, message):
    chat_id = message.chat.id
    await noobdb.deny_user(chat_id)
